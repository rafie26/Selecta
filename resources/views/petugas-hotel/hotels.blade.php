                ...
                <tbody>
                    @forelse(App\Models\RoomType::latest()->get() as $roomType)
                        <tr>
                            ...
                            <td>
                                @php
                                    $photo = $roomType->featuredPhoto ?? $roomType->activePhotos()->first();
                                    $photoUrl = $photo ? $photo->image_url : null;
                                @endphp
                                @if($photoUrl)
                                    <div class="d-flex flex-column align-items-start gap-1">
                                        <img src="{{ $photoUrl }}" alt="{{ $roomType->name }}" style="width: 80px; height: 60px; object-fit: cover; border-radius: 4px;">
                                        <button type="button" class="btn btn-link p-0 btn-sm" data-room-type-id="{{ $roomType->id }}" onclick="managePhotosForRoomType(this.dataset.roomTypeId)">
                                            Kelola Foto
                                        </button>
                                    </div>
                                @else
...
$(document).ready(function() {
    ...
    $('#adjustRoomsForm').on('submit', function(e) {
        ...
        $.ajax({
            url: `/petugas-hotel/hotels/${id}/adjust-rooms`,
            type: 'POST',
            data: {
                action: action,
    ...
    $('#confirmDelete').on('click', function() {
        if (roomTypeIdToDelete) {
            $.ajax({
                url: `/petugas-hotel/hotels/${roomTypeIdToDelete}`,
                type: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
    ...
    $('#addPhotoForm').on('submit', function(e) {
        ...
        const imageFile = $('#photo_image')[0].files[0];
        ...
        if (imageFile) {
            ...
            console.log('Image manually set in FormData');
        }
        
        // Clean up empty sort_order
        if (!formData.get('sort_order') || formData.get('sort_order') === '') {
        ...
        $.ajax({
            url: '/petugas-hotel/hotel-photos',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#editPhotoForm').on('submit', function(e) {
        ...
        $.ajax({
            url: `/petugas-hotel/hotel-photos/${photoId}`,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#confirmDeletePhoto').on('click', function() {
        if (photoIdToDelete) {
            $.ajax({
                url: `/petugas-hotel/hotel-photos/${photoIdToDelete}`,
                type: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
    ...
    function attachPhotoEventHandlers() {
        ...
        $('.toggle-featured').on('click', function() {
            ...
            $.ajax({
                url: `/petugas-hotel/hotel-photos/${photoId}/toggle-featured`,
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
        ...
        $('.toggle-status').on('click', function() {
            ...
            $.ajax({
                url: `/petugas-hotel/hotel-photos/${photoId}/toggle-status`,
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
    ...
    // Global helper to manage photos for a specific room type from the table
    window.managePhotosForRoomType = function(roomTypeId) {
        // Set filter to selected room type and reload photos
        $('#filterRoomType').val(roomTypeId).trigger('change');

        // Smooth scroll to the Kelola Foto Hotel section
        const photosCard = $('#photosGrid').closest('.card');
        if (photosCard.length) {
            $('html, body').animate({
                scrollTop: photosCard.offset().top - 80
            }, 600);
        }
    };
