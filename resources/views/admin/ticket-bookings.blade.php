...
@section('title', 'Kelola Booking Tiket')
    ...
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex gap-3 align-items-center">
                ...
                <form method="GET" action="{{ url()->current() }}" class="d-flex gap-2 align-items-center">
                    ...
                    <select name="package" class="form-select form-select-sm" style="width: auto; min-width: 150px;" onchange="this.form.submit()">
                ...
                <div class="d-flex gap-2">
                    <span class="badge bg-primary">Total: {{ $bookings->total() }} booking tiket</span>
                    @if(request()->hasAny(['search', 'payment_status', 'check_in_status', 'package']))
        ...
        @if(request()->hasAny(['search', 'payment_status', 'check_in_status', 'package']))
            <div class="mt-3 pt-3 border-top">
                ...
                @if(request('package'))
                    <span class="badge bg-light text-dark me-2">
                        Paket: {{ $packages->find(request('package'))->name ?? 'Unknown' }}
                        <a href="{{ request()->fullUrlWithoutQuery('package') }}" class="text-decoration-none ms-1">
                            <i class="fas fa-times"></i>
                        </a>
                    </span>
                @endif
                                        ...
                                        <div class="small mb-2">
                                            ...
                                            @if($detail->package->description)
                                                <div class="text-muted small">
                                                    {{ Str::limit($detail->package->description, 50) }}
                                                </div>
                                            @endif
                                        </div>
                                    @endforeach
                                @else
                                    <span class="text-muted">Tidak ada data paket</span>
                                @endif
                            </td>
                            <td>
                                @if($booking->visitors->count() > 0)
                                    <div class="small">
                                        <div class="fw-medium">{{ $booking->visitors->count() }} pengunjung</div>
                                        @foreach($booking->visitors->take(2) as $visitor)
                                            <div class="text-muted">{{ $visitor->name }}</div>
                                        @endforeach
                                        @if($booking->visitors->count() > 2)
                                            <div class="text-muted">+{{ $booking->visitors->count() - 2 }} lainnya</div>
                                        @endif
                                    </div>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-4">
                                ...
                                @if(request()->hasAny(['search', 'payment_status', 'check_in_status', 'package']))
                                    <p class="text-muted mb-0">Tidak ada booking tiket yang sesuai dengan filter</p>
                                    <small class="text-muted">
                                        <a href="{{ url()->current() }}" class="text-decoration-none">Hapus filter</a> untuk melihat semua data
                                    </small>
                                @else
                                    <p class="text-muted mb-0">Belum ada booking tiket</p>
                                @endif
                            </td>
                        </tr>
                    @endforelse
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="statusModalLabel">Update Status Booking Tiket</h5>
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="deleteModalLabel">Konfirmasi Hapus Booking Tiket</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                ...
                <p>Apakah Anda yakin ingin menghapus booking tiket berikut?</p>
                ...
                <div class="mt-3">
                    <small class="text-muted">
                        ...
                        Semua data terkait booking tiket ini akan ikut terhapus.
                    </small>
                </div>
            </div>
            <div class="modal-footer">
...
<div class="modal fade" id="bookingDetailModal" tabindex="-1" aria-labelledby="bookingDetailModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="bookingDetailModalLabel">Detail Booking Tiket</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
